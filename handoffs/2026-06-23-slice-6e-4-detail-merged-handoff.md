...
- **Owner-gated dependency:** a Logto **native app** registration (redirect URI = the custom scheme callback; bundle id `com.redducklabs.fountainrank`, owner-confirmed — see memory `fountainrank-bundle-id-confirmed`) + the external OAuth records. Track these via `docs/setup/README.md` and the `claude_help/oauth-sso.md` external-registrations checklist. Claude takes the code to Local CI; the **owner** provides the records and the **device round-trip** proves it (spec §21: 6e-5 = Local CI as code → Owner-gated + on-device for the auth/write claim).
