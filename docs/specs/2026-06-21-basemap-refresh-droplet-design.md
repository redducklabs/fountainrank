...
- **Remote script** (run over SSH — see §5 for the exact safe invocation). The script:
  1. Waits for first-boot **cloud-init** to finish and retries `apt` past the **dpkg lock** (a fresh droplet's cloud-init/apt-daily holds it), then installs **AWS CLI v2** (pinned 2.35.9, via the official installer zip — Ubuntu 24.04 has no apt `awscli` package — with the zip's **SHA256 verified** before executing the installer, since it runs with creds in env); configures **non-secret** aws settings (placeholder region, `s3.endpoint_url`, `s3.multipart_chunksize 256MB`, `s3.multipart_threshold 256MB`) via `aws configure set` — credentials are **NOT** written to `~/.aws`; they are exported into the process env only.
  ...
  3. **Resumable download:** a loop of **fresh `curl -C -` invocations** (each resumes from the partial file's current size via a Range request). This is the actual fix for the HTTP/2 reset that killed the in-runner stream: curl's own `--retry` restarts from byte 0 on a mid-transfer drop (and `-C -` fixes its resume offset once at process start), so resuming requires re-invoking curl, not `--retry`. https-only, no redirects, IP-pinned.
  ...
  5. **Upload intra-region:** file-based multipart `aws s3 cp … s3://fountainrank-basemap/planet.pmtiles` with **256 MB parts** + **`cli_read_timeout 0`** (DO Spaces' `CompleteMultipartUpload` on ~2000 small parts timed out → aws-cli retried → DO "completion already in progress", a false failure even though the object completed). The **success criterion is a post-upload size check** (`aws s3api head-object` ContentLength == `SRC_LEN`, retried for async finalization) — **not** cp's exit code. Steps 2–5 are also wrapped in an **idempotency skip**: if the live object already matches `SRC_LEN` (a prior run uploaded it but its marker wasn't recorded), download+upload are skipped.
