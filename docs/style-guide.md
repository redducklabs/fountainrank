...
### SEO place pages (`web/app/drinking-fountains/**`)
...
- **Hub country grid:** `/drinking-fountains` lists ready countries in a two-column responsive grid.
  ...
  hero treatment.
- **Country region/city list:** country pages list canonical regions when the country has a region
  tier; two-level countries list cities directly. Rows use `divide-y divide-border`, brand-colored
  underlined links, and right-aligned muted counts.
- **Breadcrumb trail + JSON-LD:** city breadcrumbs are built from the place tree: country → parent
...
- **Title:** `h1` in `text-2xl font-black text-brand-ink` ("Drinking fountains in {place}"),
  followed by a one-line lead in `text-muted` stating the fountain count (city/region pages add
  "Showing the top N" when the list is capped).
- **Fountain rows:** city and region pages render the shared `FountainList`. Fountains have no
  public names, so the label remains "Drinking fountain" (+ "· Out of order" when not working) with
  the shared rating treatment.
- **Indexability:** the backend computes place indexability from fountain count plus country route
  readiness. Pages render below the gate but set `noindex`; unknown places 404. The resolver at
  `/drinking-fountains/[country]/[place]` consumes the backend `/resolve/{slug}` decision and uses
  `permanentRedirect()` for legacy flat city URLs that now need a region segment.
...
- Placement: below community notes and above contribution controls, separated by a simple
  top border (`border-t border-border pt-4`) rather than a nested page card.
- Edit form: compact field group (`rounded-lg border border-border bg-surface p-3`)
...
- Moderation reason: one compact, 500-character textarea feeds the audit trail for hide, unhide,
  dismiss, and delete actions. It is optional for legacy moderation actions and explicitly marked
  as required for rating removal.
- Rating moderation: repeated rating rows identify the dimension, score, and contributor, with a
  compact danger-outline **Remove rating** action. The action stays disabled until a non-blank
  reason is present; success refreshes the detail so aggregates and the row list update together.
...
| `Find nearest`      | FountainRank control | Separate 44×44 map-pin-with-search glyph and tooltip/accessibility name “Find nearest fountain”. Gets a fresh position below the complete native navigation/geolocation stack with a visible gap, selects the globally nearest fountain, and opens details. It is placed outside MapLibre's native control stack so it cannot be mistaken for the crosshair-style locate-me control. Results ≤50 km open immediately; farther results require a distance-bearing confirmation. |
...
- A persistent outlined **Store** link in the right-hand cluster opens the Red Duck Labs merchandise
  store in a new tab. It uses the secondary translucent-white header treatment so it remains visible
  without competing with the gold sign-in action. Its accessible name identifies the destination and
  announces the new tab; `rel="noopener noreferrer"` isolates the external page.
...
### Points-ineligible inline warning (all contribution flows)

Shown when the signed-in viewer's award for a contribution is **already spent**, so submitting will
earn nothing. It **replaces** the Possible-points preview for that flow.

Two sources drive it:

1. **Conditions (#124)** — `FountainDetail.condition_points_eligible_at` is a future timestamp
   (the rolling 24h window). Original flow; details below.
2. **Ratings, details, comments and photos (#204)** — `FountainDetail.viewer_award_state`, computed
   from the contribution **dedup ledger** (`contribution_events.dedup_key`), i.e. the award rule
   itself. The clients read it via `ratingEarnablePoints()` / `attributeEarnablePoints()` /
   `notePointsPreview()` / `photoEarnablePoints()` in `@fountainrank/contributions`; when the
   returned line list is empty but the user has selected something, show the warning instead of the
   preview.

Copy per flow (state the **reason**, not just the fact):

| Flow      | Copy                                                                                                                    |
| --------- | ----------------------------------------------------------------------------------------------------------------------- |
| Rating    | "You've already earned points for these dimensions — you can still update your rating, but it won't earn points again." |
| Details   | "You've already earned points for these details — you can still update them, but they won't earn points again."         |
| Comment   | "You've already earned points for a comment here — you can still update it, but it won't earn points again."            |
| Photo     | "Points are only awarded for a fountain's first photo — this one won't earn points."                                    |
| Condition | see the #124 copy below                                                                                                 |

Mobile renders all of these through the shared `NoPointsNotice` in
`mobile/components/fountain/RatingContributionForm.tsx` (same amber treatment as the condition
`limitNote`). Web uses the amber inline-note classes below.

**Condition-specific detail (#124):**
...
- **`SpinnerButton`** — the standard `<button>` for `useTransition`/async-driven controls. Takes
  ...
  `inline-flex items-center justify-center gap-2` + the spinner + a11y. Pass the caller's _other_
...
respects the OS setting). The spinner **precedes** the _Contribution celebration_ below and is never
the sole success/error signal.

### Contribution celebration

A short reward animation shown after successful contribution writes **that actually awarded points**.

> **🔴 The rule (#204): the celebration fires if and only if the SERVER awarded points — i.e. the
> write response's `points_awarded > 0`. A contribution that saves but earns nothing (a re-rate, a
> re-observed detail, a 2nd comment, a non-first photo, a condition inside the 24h window) gets a
> neutral inline confirmation stating _why_ it earned nothing — and NO animation, NO number.**
>
> This is enforced in code, not by convention: `dispatchContribution()` takes a branded
> `AwardedPoints` that only the response-parsing layer can mint (`web/app/actions/awarded.ts` behind
> `server-only`; `mobile/lib/awarded-points.ts`, whose parameter is the generated API response
> union). A client-computed number is a **type error**. Both web listeners — the status overlay and
> `MapBrowser` — gate on `> 0`, as does mobile's `refreshDetailAfterWrite`.
>
> Never celebrate an award you cannot verify: an unknown/`null` `points_awarded` is treated as 0.
