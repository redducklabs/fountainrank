...
**place context** search engines can rank: pages for _drinking fountains in `<city>`_ and
_`<country>`_, built by point-in-polygon assigning every fountain to the administrative area that
...
- **Check coverage:** run **`seo-coverage-report.yml`** (manual dispatch, no inputs, read-only) — it
  ...
  ready/not-ready _recommendation_). It reads **one consistent snapshot** (it serializes with
...
### B.4 Keep the app-store URLs current (#135)

The production web build passes `NEXT_PUBLIC_APP_STORE_URL` / `NEXT_PUBLIC_GOOGLE_PLAY_URL` from
`deploy.yml`, enabling the footer links and mobile app promotion. If either listing is replaced,
update its public build argument and verify the resulting link after deployment.
