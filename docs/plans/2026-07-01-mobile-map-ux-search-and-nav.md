...
- [ ] Implement `ProfileTabIcon` as a **fetch-disabled but cache-reactive read** (spec §5.3 "no stray request" **and** "shows the photo when available"): subscribe with `useQuery({ queryKey: ["me"], enabled: false })` — a disabled observer still **subscribes to the `["me"]` cache and re-renders when `NameGate` populates/updates it**, but never triggers its own fetch (no `queryFn` invocation while disabled). This avoids the twin failure modes: a one-shot `getQueryData` read would be non-reactive (stuck on the glyph after the avatar loads), and an `enabled: true` observer would refetch (the root `QueryClient` has no `staleTime`). Read `data?.avatar_url`; render the circular `<Image>` (brand-blue ring when `focused`) or the `person-circle` glyph; image-load error falls back to the glyph. Reuse the avatar styling from `account.tsx`.
- [ ] **Add a mocked-client test asserting both properties:** (a) rendering/subscribing issues **no** `client.GET`; (b) when the `["me"]` cache goes from empty → an object with `avatar_url`, the component **re-renders glyph → image** (reactivity). (Pure `profileTabIcon` already covers the image-vs-glyph decision.)
...
- [ ] **Add a generic map-press seam for marker clearing.** Today `FountainMap`'s `Map.onPress` handler only serves add-mode: it early-returns when `onMapPressForPlacement` is absent (the normal search/browse case), so a plain tap notifies nobody. Add an `onMapPress?: () => void` prop that fires on **every** map press (invoked before/independent of the add-placement early-return), leaving the existing `onMapPressForPlacement` add-mode behavior unchanged. `index.tsx` passes `onMapPress={() => clearSearchMarker via shouldClearSearchMarker({ cause: "press" })}` so a non-add-mode tap clears the search marker without entering placement. Verify at the call site that add-mode placement still works (both props can be set; `onMapPress` never triggers placement).
