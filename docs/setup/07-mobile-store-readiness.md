...
5. Use the configured Android production submit track `internal` unless the owner
   chooses closed testing for the first broader group. CI uploads Android
   internal-testing submissions as Play drafts because the EAS Android submit
   options do not carry release-note text.
6. Copy the PR-only release notes from the `mobile-store-release.yml` run summary
   into the Play release notes field before completing the draft release.
...
| Store release notes            | Generated by `.github/workflows/mobile-store-release.yml` from squash-merged PR subjects in the release range. iOS sends the list to TestFlight "What to Test"; Android leaves the Play release as a draft and the owner must paste the same list into Play release notes before completing the release.                          | Confirm the notes match the PRs included in the released build.                                         |
