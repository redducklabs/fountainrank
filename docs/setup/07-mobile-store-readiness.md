...
Store app versions default to `1.0.3` in `mobile/app.config.ts` (bump for every
...
| Screenshots                    | `mobile/assets/store/screenshots/play-store/` holds **real device captures** (release-config app on the Android emulator, San Diego, `1080x1920`): map discovery, fountain detail, address/city search, rating filter, rankings. The App Store 6.9" (`1290x2796`) and 6.5" (`1242x2688`) sets are still **generated mockups** pending real iPhone captures (no macOS in the build env). Rating/ranking values shown are seed/test data.                             | Review in the consoles. Replace the iOS mockups with physical-device captures before final submission.  |
...
| Splash image                     | `mobile/assets/splash-icon.png`                       | Configured through the SDK-57 `expo-splash-screen` config plugin.                                                                                                    |
...
| App Store 6.9 screenshots        | `mobile/assets/store/screenshots/app-store-6-9/*.png` | 1290x2796 RGB PNGs for the iPhone 6.9" Display slot in App Store Connect. **Generated mockups** — replace with real iPhone captures before final submission.         |
