import { getSitemapCitiesServer, SITEMAP_CITY_CAP } from "../../lib/places";
...
// Dynamic because the city sitemap chunk count comes from the live backend total_count.
export const dynamic = "force-dynamic";

export async function GET(): Promise<Response> {
  const requestId = crypto.randomUUID();
  const cities = await getSitemapCitiesServer(requestId, 1, 0);
  if (!cities.data) {
