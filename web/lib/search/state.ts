// Pure state helpers for the web header search (design doc
// docs/specs/2026-07-01-web-search-and-mobile-polish-design.md §4.1). No React, no
// network - HeaderSearch (a later task) owns dispatch, debouncing, and the actual
// `AbortController`/fetch; this module only decides what the UI should show and
// which stale responses to ignore. Mirrors mobile/lib/map-search/state.ts (the
// proven pattern) with one addition: SearchResultItem carries an optional
// `boundingBox` so a selected result can drive `deriveCameraAction` (see flyto.ts).
...
/**
 * v1 has a single error condition end-to-end ("Search is unavailable right now")
 * covering a 503/502/429/network failure alike - see `search/geocode-client.ts`
 * `mapGeocodeError`. Kept as a union (not a bare string literal) so a future
 * distinct reason is a type-level addition, not a silent string typo.
 */
...
export type SearchResultItem = {
  ...
  latitude: number;
  longitude: number;
  /** Present only when the backend returned a valid, positive-area extent (§2). */
...
/**
 * The key the caller's debounce effect should key off of, instead of the raw
...
export function searchReducer(state: SearchState, action: SearchAction): SearchState {
  switch (action.type) {
    ...
    case "resultsReceived": {
      // Stale-response guard: a response is applied only if its seq is the
      // latest dispatched - an older/slower response never overwrites newer
      // results.
